...
5. Never commit the results of `notes.mjs` / `pr-feed.mjs` in isolation — the side-effect config files (`.docsearch/config.json`, `pr-feed-config.json`, `setup.js`, `sidebar-menus/history.ts`, `navbar-menus/about.js`) must be staged in the same commit.
