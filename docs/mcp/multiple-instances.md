...
If you need to switch between more than one Rundeck instance (staging and production, say) in the same session, without quitting and reconfiguring the server, configure `RUNDECK_INSTANCES` instead.
...
If you're running the server outside a client's managed config (invoking `claude` directly from a terminal, say), export it in your shell first:
