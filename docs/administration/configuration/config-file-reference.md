...
Options not declared on a job would otherwise bypass all server-side option validation yet still reach the option data context and `RD_OPTION_*` environment variables. When this control is enabled, such executions are created and then failed at start before any workflow step runs. Enable it to prevent unvalidated option names from reaching scripts. The default is `false`, which allows undeclared options to pass through. Scope: top-level executions (UI, API, webhook, and scheduled).
