...
This User Class allows full access to the Rundeck environment.
...
>_Note: “None” would represent no access to anything in Rundeck.  The user could login but would have access to nothing.  This is not an assignable class today and will not show up in the management UI.  It is only available for default assignment._
...
1. Login to Rundeck as an admin equivalent account.
